...
def _request(method, url, headers=None, payload=None, params=None, timeout=15):
    ...
    try:
        ...
        raise ApiError(f"ط®ط·ط§غŒ ط§طھطµط§ظ„ ط¨ظ‡ ط³ط±ظˆط±: {exc.reason}") from exc
    except TimeoutError as exc:
        raise ApiError("ط²ظ…ط§ظ† ط§طھطµط§ظ„ ط¨ظ‡ ط³ط±ظˆط± ط¨ظ‡ ظ¾ط§غŒط§ظ† ط±ط³غŒط¯.") from exc
...
class SupabaseClient:
    ...
    def sign_in(self, identifier, password):
        if not self.configured:
            raise ApiError("ط§طھطµط§ظ„ ط³ط±ظˆط± ط¯ط± طھظ†ط¸غŒظ…ط§طھ ط§غŒظ† ظ†ط³ط®ظ‡ ظپط¹ط§ظ„ ظ†ط´ط¯ظ‡ ط§ط³طھ.")

        identifier = (identifier or "").strip()
        if not identifier or not password:
            raise ApiError("ظ†ط§ظ… ع©ط§ط±ط¨ط±غŒ ظˆ ط±ظ…ط² ط¹ط¨ظˆط± ط±ط§ ظˆط§ط±ط¯ ع©ظ†غŒط¯.")
        ...
        if not response.ok:
            raise ApiError(
                self._error(response, "ط§غŒظ…غŒظ„ غŒط§ ط±ظ…ط² ط¹ط¨ظˆط± طµط­غŒط­ ظ†غŒط³طھ.")
            )
        ...
        if not self.access_token:
            self.access_token = ""
            self.refresh_token = ""
            raise ApiError("ط³ط±ظˆط± ظ†ط´ط³طھ ظ…ط¹طھط¨ط± ط§غŒط¬ط§ط¯ ظ†ع©ط±ط¯.")
    ...
    def _profile(self, user):
        ...
        for key in ("role", "display_name", "full_name", "username"):
            if key in metadata:
    ...
    def table_select(self, table, params=None):
        if not self.configured:
            raise ApiError("ط§طھطµط§ظ„ ط³ط±ظˆط± ظپط¹ط§ظ„ ظ†غŒط³طھ.")

        if not self.access_token:
            raise ApiError("ظ†ط´ط³طھ ظ…ط¹طھط¨ط± ظ†غŒط³طھ.")
    ...
    def _error(self, response, default="ط®ط·ط§غŒ ط³ط±ظˆط±"):
        try:
